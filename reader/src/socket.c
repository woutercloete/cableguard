...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint16 flag) {
  ...
  printf("socket()\r\n");
#endif
  if ((protocol == Sn_MR_TCP) || (protocol == Sn_MR_UDP) || (protocol == Sn_MR_IPRAW)
...
uint8 connectSocket(SOCKET s, uint8 * addr, uint16 port) {
...
uint32 sendSocket(SOCKET s, uint8 * buf, uint32 len) {
  uint8 status = 0;
  uint32 ret = 0;
  uint32 freesize = 0;
#ifdef __DEF_IINCHIP_DBG__
 ...
 *********************************************************************
 */
uint32 recvSocket(SOCKET s, uint8 * buf, uint32 len) {
  uint16 pack_size = 0;
 ...
 *********************************************************************
 */
uint16 sendtoSocket(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 port) {
  uint8 status = 0;
  uint8 isr = 0;
  uint16 ret = 0;
 ...
 *********************************************************************
 */
uint16 recvfromSocket(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port) {
  uint8 head[8];
  uint16 data_len = 0;
...
uint16 igmpsendSocket(SOCKET s, uint8 * buf, uint16 len) {
  uint8 status = 0;
  uint8 isr = 0;
  uint16 ret = 0;
