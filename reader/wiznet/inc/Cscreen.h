...
class Ctagscreen: public Cscreen {
  ...
  public:
    ...
    void build(C1101::sRadioPacket* pkt, u08 filRssi, u08 rssiReject) {
      ...
      memset(&screen, 32, sizeof(screen));
      snprintf((c08*) &screen.lines[0][0], screenMaxCol, "#%04X%04X", (u16) (pkt->tag.tagID >> 16),
               (u16) pkt->tag.tagID);
      //snprintf((c08*) &screen.lines[1][0], "RSSI: %d %d", pkt->rssi, pkt->tag.count);
      snprintf((c08*) &screen.lines[1][0], screenMaxCol, "RSSI: %d %d %d", pkt->rssi, filRssi,rssiReject);
      Cscreen::build();
    }
    /****************************************************************************************/
    void build(void) {
